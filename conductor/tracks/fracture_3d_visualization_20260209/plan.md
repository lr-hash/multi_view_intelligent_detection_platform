...
- [x] **Task: 开发压裂球渲染工具**
    - [x] 在 `frontend/src/utils/three-utils.js` 中添加 `createFractureSphere` 函数.
- [x] **Task: 集成动态映射逻辑**
    - [x] 在 `VisualizeView.vue` 中调用 API 获取数据.
    - [x] 根据压力和流量计算球体的 scale 和 color.
- [x] **Task: 实现点击交互**
    - [x] 恢复 Raycaster 逻辑，支持选中压裂球并展示详情.
- [x] **Task: Conductor - User Manual Verification '动态 3D 压裂段渲染' (Protocol in workflow.md)**
